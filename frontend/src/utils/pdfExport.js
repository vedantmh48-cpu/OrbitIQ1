...
// --- Brand logo -------------------------------------------------------------
// The artwork lives in /public and is embedded as a data URL, so exported PDFs
...
function loadLogo() {
  if (!logoPromise) {
    logoPromise = fetch(LOGO_URL)
      ...
      .catch(() => null); // export must still work without the artwork
  }
  return logoPromise;
}
...
function drawLogo(doc, logo, { right, top, height = 10 }) {
  ...
  doc.roundedRect(x - 2.5, top - 2.5, width + 5, height + 5, 1.5, 1.5, "F");
  const format = String(logo).startsWith("data:image/png") ? "PNG" : "JPEG";
  try {
    doc.addImage(logo, format, x, top, width, height, undefined, "FAST");
  } catch {
    /* unsupported artwork in this browser — keep the text-only header */
  }
}

/** One line that fits `maxWidth` (mm) at the currently selected font. */
function fitLine(doc, text, maxWidth) {
  const lines = doc.splitTextToSize(String(text || ""), maxWidth);
  if (lines.length <= 1) return lines[0] || "";
  return `${String(lines[0]).replace(/\s+\S*$/, "")}…`;
}
...
export async function buildAnalysisPdf(result) {
...
// Methodology + meta
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(THEME.grey[0], THEME.grey[1], THEME.grey[2]);
    doc.text(`Generated by SatQuery AI · ${(result.created_at || "").slice(0, 10)}`, M, 292);
    doc.text(`Page ${i} / ${pages}`, W - M, 292, { align: "right" });
  }

  const filename = `satquery-${result.id ? result.id.slice(0, 8) : "report"}.pdf`;
 ...
 */
export async function buildChangeAnalysisPdf(result) {
  const logo = await loadLogo();
  const doc = new jsPDF({ unit: "mm", format: "a4", compress: true });
